  ...
  return (
    <div class="space-y-8">
      <section class="rounded-2xl border border-line bg-surface p-6 shadow-soft">
        <p class="text-sm font-medium text-accent">Qwik WebClient</p>
        ...
        <div class="mt-5 flex flex-wrap gap-2">
          <Link class="rounded-lg bg-ink px-4 py-2 text-sm font-semibold text-white" href="/projects">Open projects</Link>
          <Link class="rounded-lg border border-line px-4 py-2 text-sm font-semibold" href="/api-health">Check API health</Link>
        </div>
      </section>
      <section>
        <h3 class="mb-3 text-lg font-semibold">Resource overview</h3>
        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {resourceMetadata.map((resource) => <Link key={resource.key} href={resource.routePath} class="rounded-xl border border-line bg-surface p-4 shadow-sm hover:shadow-soft"><div class="flex items-center justify-between"><span class="font-semibold">{resource.pluralLabel}</span><span class="rounded-full bg-raised px-2 py-1 text-xs text-muted">{counts.value[resource.key] ?? '—'}</span></div><p class="mt-2 text-sm text-muted">{resource.description}</p></Link>)}
        </div>
      </section>
    </div>
  );
